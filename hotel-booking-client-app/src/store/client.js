...
const useRoomStore = defineStore("client", {
  state: () => ({
    message: "",
    records: [],
    record: null,
  ...
  actions: {
    async fetchRecords() {
      try {
        ...
        const request = await axios.get(`${generalStore.API_URL}client`, {
          headers: {
        ...
        /* * /
        console.log(
          "store client - fetchRecords # " + JSON.stringify(this.records, null, 2)
        );
        /* */
      } catch (error) {
        const userStore = useUserStore();
    ...
    async fetchRecordById(id) {
      ...
      const userStore = useUserStore();
      const token = userStore.token;
      const getAPI = `${generalStore.API_URL}client/${id}`;
      ...
      await axios({
        ...
        .then((response) => {
          ...
          /*
          console.log(
            "store client - this.record # " + JSON.stringify(this.record)
          );
          */
        })
        .catch((error) => {
    ...
    async editRecord(isEditCheck, form) {
      ...
      const userStore = useUserStore();
      const token = userStore.token;
      const addAPI = `${generalStore.API_URL}client`;
      const editAPI = `${generalStore.API_URL}client/${form.id}`;
    ...
    async deleteRecord(id) {
      ...
      const userStore = useUserStore();
      const token = userStore.token;
      const deleteAPI = `${generalStore.API_URL}client/${id}`;
